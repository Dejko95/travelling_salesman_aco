...
public class AntSystem4 {
	public static int lastUpdateIter = 0;
	public static final double TMIN = 0.0000015, TMAX = 0.05;
	...
	public static double distance(Point points[]) {
		...
		for (int i=0; i<n; i++) {
			for (int j=0; j<n; j++) {
				//t[i][j] = TMAX;
				t[i][j] = 1 / Greedy2.getDistance(points) / p;
		...
		while (iter-- > 0) {
			...
			if (iter % 2 == 0) {
				...
				for (int i=0; i<n; i++) {
					int x = path[best][i];
					int y = path[best][(i+1) % n];
					t[x][y] += 1/len[best];
					if (t[x][y] > TMAX) t[x][y] = TMAX;
				}
			}
			else {
				for (int i=0; i<n; i++) {
